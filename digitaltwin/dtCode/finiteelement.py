...
def Submit():
    ...
    # Write Script
    path = os.path.join("digitaltwin","dtData","ABAQUS_Scratch","RunScript.py")
    try:
        f=open(path,'w')
    except:
        f=open(path,'x')
    ...
    # Add verbose
    f=open(path,'a')
    pathg = os.path.join("digitaltwin","dtData","FEAVerbose.txt")
    g=open(pathg,'r')
    f.write(g.read())
    g.close()
    f.close()
    ...
    # Run Abaqus
    dirname= os.path.join("digitaltwin","dtData","ABAQUS_Scratch")
    os.chdir(dirname)
...
def write_abaqus():
    ...
    ft_mesh=req.get('ft_mesh')
    # Write Script
    path_like = os.path.join("digitaltwin","dtData","ABAQUS_Scratch")
    filename = "RunScript.py"
    path = os.path.join(path_like,filename)
    try:
        f=open(path,'w')
    except:
        f=open(path,'x')
    ...
    return send_file(os.path.join("dtData","ABAQUS_Scratch",filename),as_attachment=True)